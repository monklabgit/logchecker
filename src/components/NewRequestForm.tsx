...
import { AlertCircle, Camera, Check, FileUp, LoaderCircle, Minus, Plus, Save, ScanText, Trash2, Upload, X } from 'lucide-react';
...
const itemsFromExtraction = (items?: AiMaterialItem[]) =>
  items?.length
    ? items.map((item) => ({
        id: crypto.randomUUID(),
...
export function NewRequestForm({ onSaved }: NewRequestFormProps) {
  ...
  useEffect(() => {
    ...
    loadHospitals().then((nextHospitals) => {
      if (!active) return;
      setHospitals(nextHospitals);
    });
  ...
  const updateItem = (section: SectionName, id: string, key: keyof Omit<MaterialItem, 'id'>, value: string) => {
    setItems(section, getItems(section).map((item) => (item.id === id ? { ...item, [key]: value } : item)));
  };
  ...
  const applyExtraction = (result: ExtractionResult) => {
    ...
    setCmeItems(itemsFromExtraction(result.cmeItems));
    setOpmeItems(itemsFromExtraction(result.opmeItems));
  ...
  const saveRequest = async () => {
    ...
    const items = [
      ...
      .map(({ section, quantity, description, note }) => ({ section, quantity, description, note }));
